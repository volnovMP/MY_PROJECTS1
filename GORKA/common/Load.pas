  ...
  try
    ...
    begin
      ...
      begin
        ...
        if ObjView[start+j].Points[k].X > 1600 then
        begin
          if ((start+j) = 738 )  then ObjView[start+j].Points[k].X :=ObjView[start+j].Points[k].X - 1610
          else
          if ((start+j) = 706 ) then ObjView[start+j].Points[k].X :=ObjView[start+j].Points[k].X - 1610
          else  ObjView[start+j].Points[k].X := ObjView[start+j].Points[k].X - 1630
        end
        else ObjView[start+j].Points[k].X :=0;
        if getinteger(i, s, ObjView[start+j].Points[k].Y) then break;       //чтение координаты У
        ObjView[start+j].Points[k].Y := ObjView[start+j].Points[k].Y + 200;

        if (start+j) = 846 then
        begin
          ObjView[843].Points[k].X := ObjView[843].Points[k].X - 200;
          ObjView[844].Points[k].X := ObjView[844].Points[k].X - 200;
          ObjView[845].Points[k].X := ObjView[845].Points[k].X - 200;
          ObjView[846].Points[k].X := ObjView[846].Points[k].X - 200;

          ObjView[843].Points[k].Y := ObjView[843].Points[k].Y - 100;
          ObjView[844].Points[k].Y := ObjView[844].Points[k].Y - 100;
          ObjView[845].Points[k].Y := ObjView[845].Points[k].Y - 100;
          ObjView[846].Points[k].Y := ObjView[846].Points[k].Y - 100;
        end;
      end;
    ...
    if not fullrecord then begin ShowMessage('Ошибка при раcпаковке параметров фрагмента описания зависимостей '+ filepath); result := false; exit; end;
        ObjView[739].TypeObj :=0;
  ObjView[709].TypeObj :=0; ObjView[710].TypeObj :=0;  ObjView[711].TypeObj :=0;  ObjView[175].TypeObj :=0;
  ObjView[176].TypeObj :=0; ObjView[320].TypeObj :=0;  ObjView[291].TypeObj :=0;  ObjView[273].TypeObj :=0;
  ObjView[272].TypeObj :=0; ObjView[322].TypeObj :=0;  ObjView[323].TypeObj := 0; ObjView[316].TypeObj :=0;
  ObjView[733].TypeObj :=0; ObjView[293].TypeObj :=0;  ObjView[321].TypeObj :=0;  ObjView[292].TypeObj :=0;
  ObjView[752].TypeObj :=0; ObjView[717].TypeObj :=0;  ObjView[507].TypeObj :=0;  ObjView[508].TypeObj :=0;
  ObjView[505].TypeObj :=0; ObjView[506].TypeObj :=0;  ObjView[753].TypeObj :=0;  ObjView[707].TypeObj :=0;
  ObjView[847].TypeObj := 4; ObjView[847].RU := 1; ObjView[847].Layer := 1;
  ObjView[847].Points[2].X := 250; ObjView[847].Points[2].Y := 520;
  ObjView[847].Points[3].X := 290; ObjView[847].Points[3].Y := 520;

  ObjView[848].TypeObj := 4; ObjView[848].RU := 1; ObjView[848].Layer := 1;
  ObjView[848].Points[2].X := 410; ObjView[848].Points[2].Y := 560;
  ObjView[848].Points[3].X := 450; ObjView[848].Points[3].Y := 560;

  ObjView[849].TypeObj := 4; ObjView[849].RU := 1; ObjView[849].Layer := 1;
  ObjView[849].Points[2].X := 630; ObjView[849].Points[2].Y := 720;
  ObjView[849].Points[3].X := 670; ObjView[849].Points[3].Y := 720;

  ObjView[843].Title := 'АРМ ДСПГ РПЦ ст.Орск' ;
  for i := 501 to 543 do ObjView[i].TypeObj :=0;
  for i := 745 to 747 do ObjView[i].TypeObj := 0;
  ObjView[717].TypeObj :=0;
  ObjView[742].TypeObj := 0;
    result := true;
  finally
    memo.Free;
  end;

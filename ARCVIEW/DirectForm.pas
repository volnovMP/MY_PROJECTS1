...
uses Windows, SysUtils, Classes, Graphics, Forms, Controls, StdCtrls,
  Registry, Buttons, ComCtrls, DateUtils, Dialogs;
...
var
  ...
  SoobBegin : Boolean; // 1 байт заголовка сообщения
...
procedure TDirectFormDlg.BtnOpenClick(Sender: TObject);
  var dtn,dte : Double; i,hf : integer; len,hlen , rlen : cardinal;
begin
  arhiv := ''; // сбросить текущий архив

  dtn := DateStart.Date; dte := DateEnd.Date;
  if (Trunc(dte) - Trunc(dtn)) > 1 then begin ShowMessage('Длина запрашиваемого фрагмента превышает 2-е суток'); exit; end;
  if (Trunc(dte) - Trunc(dtn)) < 0 then begin ShowMessage('Конец фрагмента определен не верно (предшествует началу фрагмента).'); exit; end;
  ...
  i := FindDateTimeFromArc(arhiv,DTFrameOffset,2); // получить индекс символа, с которого начинается ближайшая реперная точка
  if i > 1 then
  begin
    hf := 1;
...
function FindDateTimeFromArc(s : string; dt : Double; FindMode : Byte) : integer;
  var i,j : integer; DTime : Double;
begin
  result := -1; SoobCount := 0; ReperAdr := -1; TimeSoob := 0; SoobAdr := -1;
  SoobBegin := false; SoobHdr := false; SoobLen := false; SoobType := false; SoobTime1 := false; SoobTime2 := false; SoobTime3 := false; SoobTime4 := false;
  d := ''; j := 0;
  ...
  begin
    ...
    begin // начало
      SoobBegin := true; inc(SoobCount); j := 0;
    end;
    if SoobBegin then begin SoobHdr := true; SoobBegin := false; SoobAdr := i; end else
    if SoobHdr then begin SoobLen := true; SoobHdr := false; end else
    if SoobLen then begin LenSoob := byte(s[i]); SoobType := true; SoobLen := false; inc(j); end else
    if SoobType then
    begin
      TypeSoob := byte(s[i]); SoobTime1 := true; SoobType := false; inc(j);
      case TypeSoob of
        11 : ReperAdr := SoobAdr; // сохранить указатель на первую реперную точку
        12..32 : if ReperAdr < 1 then ReperAdr := SoobAdr; // сохранить указатель на последнюю реперную точку
      end;
    end else
    if SoobTime1 then begin TimeSoob := byte(s[i]); SoobTime2 := true; SoobTime1 := false; inc(j); end else
    if SoobTime2 then begin TimeSoob := TimeSoob + byte(s[i]) * $100; SoobTime3 := true; SoobTime2 := false; inc(j); end else
    if SoobTime3 then begin TimeSoob := TimeSoob + byte(s[i]) * $10000; SoobTime4 := true; SoobTime3 := false; inc(j); end else
    if SoobTime4 then begin TimeSoob := TimeSoob + byte(s[i]) * $1000000; SoobCont := true; SoobTime4 := false; inc(j); end else
    if SoobCont then
    begin
      ...
      begin // сообщение прочитано полностью
        ...
        case FindMode of
          1 : begin // поиск точки с указанной датой
            if DTime > dt then
            begin
              result := SoobAdr; exit;
            end;
          ...
          2 : begin // поиск ближайшей реперной точки
            if DTime > dt then
            begin
              result := ReperAdr; if result < 1 then result := SoobAdr; exit;
            end;
          end;
        end;
      // переход к следующему сообщению
        SoobBegin := false; SoobHdr := false; SoobLen := false; SoobType := false; SoobTime1 := false;
        SoobTime2 := false; SoobTime3 := false; SoobTime4 := false; SoobCont := false; j := 0;
      end;
    end;
  end;
end;
...
function FindStep(index : Integer) : Integer;
  var i : integer; len : byte; cd : double; lt,c : cardinal;
begin
  result := index;
  ...
  begin
    ...
    begin
      ...
      begin
        lt := byte(arhiv[i+4]);
        c := byte(arhiv[i+5]) * $100; lt := lt + c;
        c := byte(arhiv[i+6]) * $10000; lt := lt + c;
        c := byte(arhiv[i+7]) * $1000000; lt := lt + c;
        cd := lt / (60*60*24); // время фрагмента
...
begin
  ...
  if arhiv[i] <> #$ff then
  begin NeedStep := false; ShowMessage('ошибка при распаковке архива'); result := false; exit; end;
  inc(i);
  if arhiv[i] <> #0 then
  begin NeedStep := false; ShowMessage('ошибка при распаковке архива'); result := false; exit; end;
  inc(i);
...
function ArcStep(index : Integer) : Integer;
  var len,p,step : byte; i,j,ifr,cmd,imsg : integer; f4 : boolean; lt,c : cardinal; w : word;
begin
  if index > (Length(arhiv)-10) then
  begin NeedStep := false; ShowMessage('Конец фрагмента архива'); result := Length(arhiv); exit; end;

  i := index;
  if arhiv[i] <> #$ff then
  begin
    NeedStep := false; ShowMessage('ошибка при распаковке архива');
    result := FindStep(index); exit;
  end;
  inc(i);
  if arhiv[i] <> #0 then
  begin
  ...
  case TypeLastFrame of
    1,2,3,4,5,6 : begin
      step := 1; ifr := 0; f4 := false; imsg := 0;
    end;
    11..42 : begin // определить начальный адрес записи реперной точки
      ifr := 125 * (TypeLastFrame-11)+1; f4 := false; step := 0; imsg := 0;
    end;
  else
    ifr := 0; f4 := false; step := 0; imsg := 0;
  end;

  while j < len do
  begin
    case TypeLastFrame of
      1,2 : begin // распаковать ТС
        ...
        case step of
          ...
          if (ifr > 0) and (ifr < 4096) then FR3[ifr] := byte(arhiv[i]) else
...
// ?
      ...
      6 : begin // Распаковка текстовых сообщений
        case step of
          1 : ifr := byte(arhiv[i]);
          2 : ifr := ifr + byte(arhiv[i]) * $100;
          3 : imsg := byte(arhiv[i]);
      ...
      11..42 : begin // распаковка реперной записи
        p := byte(arhiv[i]);
        if f4 then begin FR4[ifr] := p; inc(ifr); f4 := false; end else begin FR3[ifr] := p; f4 := true; end;
      end;
    end;
    inc(j); inc(i);
  end;

  ...
  case ObjZav[index].TypeObj of
    ...
    51 : begin // Дополнительные датчики
      ...
      for i := 1 to 30 do
        ...
        begin
          for j := 1 to High(LinkFR3) do if LinkFR3[j].FR3 = ObjZav[o].ObjConstI[i] then begin s := LinkFR3[j].Name; break; end;
          nep := false; rdy := true; st := GetFR3(ObjZav[o].ObjConstI[i],nep,rdy);
          if nep then ValueListDlg.ValList.Values[s] := 'непарафазность' else if not rdy then ValueListDlg.ValList.Values[s] := 'нет данных' else
          if st then ValueListDlg.ValList.Values[s] := 'включен' else ValueListDlg.ValList.Values[s] := 'отключен';
        end;
    end;

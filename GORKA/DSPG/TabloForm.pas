...
begin
  ...
  Caption := 'АРМ ДСПГ - Контроль станции в реальном времени';
...
begin
  ...
  begin
    if (ObjView[i].Points[1].X = 0) and (ObjView[i].Points[2].X = 0) then ObjView[i].TypeObj :=0;
...
begin
  ...
  case Key of
    ...
    VK_F4 :
    begin
      if Shift = [] then SaveDiagnoze(config.arcpath+ '\stat.ini'); // сохранить статистику
    end;
...
begin
  ...
  try
    ...
    // определить остановку обмена по каналу АРМ-ДЦ
      ...
      // Перерисовка экрана
      if not RefreshTablo then begin DateTimeToString(s, 'dd/mm/yy h:nn:ss.zzz', LastTime); reportf('Сбой регенерации табло '+ s); end;
    ...
    begin // буфер команд заполнен
      ...
      begin // есть блокировка команд - отложить выдачу команд на сервер
        ...
        begin
          ...
          begin // превышено время ожидания команды автоматического захвата управления
            CmdCnt := 0; WorkMode.CmdReady := false; StartRM := false; // сброс команд
          end;
        end else
      ...
      begin // проверить время ожидания выдачи команды на сервер
        ...
        begin // превышено время ожидания команды
          CmdCnt := 0; WorkMode.CmdReady := false; // сброс команд
...
begin
  if Assigned(ValueListDlg) then if ValueListDlg.Visible then UpdateValueList(ID_ViewObj); // вывести параметры объекта
end;

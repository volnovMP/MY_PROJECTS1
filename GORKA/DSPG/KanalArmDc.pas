...
begin
  ...
  begin
    if LinkTCDC[i].Index[1] > 0  then begin b := GetFR3dc(LinkTCDC[i].Index[1]);  c := ((FR3dc[i,1] and  $1) = $1);  if b <> c then FR3dcn[i] := true; if b then d := (FR3dc[i,1] or $1)  else d := (FR3dc[i,1] and ($ff-$1));  FR3dc[i,1] := d; end;
